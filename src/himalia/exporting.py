"""Export a self-contained function without overwriting existing definitions."""
...
def save_function(code: str, path: str | Path, function_name: str, *, backend="auto") -> Path:
    if (
    ...
    target = Path(path)
    if target.suffix != ".py":
        raise ValueError("Export path must end in .py.")
    ...
    try:
        ...
        raise ValueError(
            "Existing export module is not valid Python; it was not changed."
        ) from None
    if function_name in symbols.get_identifiers():
        raise ValueError(
            f"Name {function_name!r} already exists; choose a different function_name."
        )
    tree = ast.parse(code)
    tree.body[0].name = function_name
    if backend != "plotly":
        # Reproduce the runtime style contexts in standalone source, without Himalia.
        preamble = "import matplotlib as himalia_mpl\n"
        context = "himalia_mpl.rc_context()"
        if backend in ("auto", "seaborn"):
            preamble += "import seaborn as himalia_sns\n"
            context += (
                ", himalia_sns.axes_style('whitegrid'), himalia_sns.plotting_context('notebook')"
            )
    ...
    content += "# Generated by Himalia. Review before running on new data.\n" + source
