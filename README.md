...
**You produce the data and model results; Himalia helps communicate them.**
...
generating new predictions belong upstream. Himalia can highlight the highest observed
...
as out of scope, Himalia raises `ScopeError` without executing code or retrying it.
...
Select **Python (Himalia)** and run cells with **Shift+Enter**. The first cell selects
...
The export runs without Himalia or an LLM, using the plotting libraries it imports:
...
| `model` | `HIMALIA_MODEL` | LLM provider/model identifier. |
| `api_base` | `HIMALIA_API_BASE` | Custom or local inference endpoint. |
| `display_format` | `"retina"` | Static output: `"retina"`, `"svg"`, or `"png"`. |
| `cache_dir` | `".himalia/plots"` | Persistent history location; `None` disables it. |
...
**Keep `.himalia/` with your notebook—the `.ipynb` alone does not contain the history.**
...
Inference uses LiteLLM. Set `HIMALIA_MODEL` and the corresponding credentials:
...
require `HIMALIA_LIVE_TEST=1` and provider configuration, and incur API usage.
